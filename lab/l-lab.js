...
MODES.ascii = {
  label: 'ascii',
  note: 'Л идёт вверх, камера держит её в центре — поэтому река едет сверху вниз. Перед буквой вода встаёт бугром и расходится вдоль бортов, у бортов проседает, позади остаётся волновая тень, а с углов по очереди срываются вихри: косой борт срывает сильнее прямого. С носа непрерывно сходят волновые фронты — они живут в воде сами и складываются в клин. «Симметрия» ставит вершину посередине, и обе грани начинают срывать поровну: видно, что несимметричный след — заслуга именно Л. «Буква» убирает тело целиком. «Бурун» — сила вала у встречного борта: он стоит узкой полосой по обшивке, сразу за ним провал, и берёт его та грань, что стоит поперёк хода. «Размах» задаёт силу воды, «срез» съедает спокойную гладь снизу и оставляет одни гребни, а «штрихи» редкой сеткой показывают отклонение от сноса там, где вода поворачивает: у косой грани, на срывах и в вихрях. Стрелка вниз посреди реки, которая и так вся едет вниз, ничего не сообщает, поэтому ровный снос остаётся тоном. «Блоки» меняют набор: у пунктуации плотный конец обрывается на @, у геометрии ступень — это доля закрашенной клетки, и штрихи показывают все восемь направлений треугольниками.',
  cursor: 'crosshair',
  tools: [
