...
function stepGame() {
  ...
  modeState.pace = 1 + level * 0.1;
  modeState.churn = 1 + level * 0.15;
  modeState.grow = Math.min(1 + level * 0.05, 2.4);
  stepSwirl();
  ...
  if (!modeState.over) {
    ...
    const beat = Math.max(0.9 * Math.pow(0.94, level), 0.22);
    if (modeState.drop >= beat) {
      ...
      const harm = Math.min(0.16 + level * 0.035, 0.55);
      const roll = Math.random();
      const kind = roll < harm ? 'крест' : roll < harm + (1 - harm) * 0.62 ? 'клин' : 'круг';
      const bunch = kind === 'крест' && level >= 8 && Math.random() < 0.45 ? (level >= 15 ? 3 : 2) : 1;
...
MODES.game = {
  label: 'игра',
  note: 'Та же река и та же буква, но по течению плывут вещи. Стрелки ведут Л: влево-вправо поперёк реки, вверх-вниз — прибавить и убавить ход. Клин — очко, крест — жизнь, кругляш не даёт ничего и стоит в наборе нарочно: без нейтральной вещи выбор свёлся бы к «хватай всё, что не крест». Жизней три. Ничего не падает по расписанию: вещи несёт то же поле, что и трассеры, поэтому у носа они обтекают букву, а в вихрях заворачивают — крест можно объехать, но можно и подвести к себе течением. Каждые шесть очков — новый вал, и давит он с четырёх сторон: река ускоряется, вещи идут чаще, крестов среди них больше и река становится вихрастее — вещи носит непредсказуемее. Доля крестов упирается в половину с небольшим, иначе ловить станет нечего; дальше давит кучность — кресты идут парами, а с пятнадцатого вала тройками, и между ними надо продеться. И сама Л прибавляет в росте: мишень крупнее, увернуться труднее — этим игра и кончается. Красным помечено происшествие, а не положение дел: удар о крест держится полсекунды и гаснет. «Заново» и клавиша R начинают сначала.',
  cursor: 'crosshair',
  tools: [
    { type: 'range', key: 'speed', label: 'течение', min: 0.15, max: 1.8, step: 0.05, value: 0.65 },
