...
function letterBody() {
  /* В игре буква прибавляет в росте с каждым валом: мишень становится
     крупнее, и увернуться от креста труднее — этим игра и кончается.
     В остальных режимах множителя нет и читается единицей. */
  const grow = modeState.grow || 1;
  const height = num('rise') * 2.4 * grow;
  const spread = num('spread') * grow;
  ...
  /* Стрелками тело ездит поперёк реки и немного вверх-вниз; ход, крен и
     высота живут в состоянии режима, поэтому в режимах без них читаются
     нулём. */
  const slide = modeState.slide || 0;
  const lift = modeState.lift || 0;
  const apex = { x: apexX + slide, y: 0.5 + height * 0.5 + lift };
  const right = { x: 0.5 + spread * 0.5 + slide, y: foot + lift };
  const left = { x: 0.5 - spread * 0.5 + slide, y: foot + lift };
  ...
  const heel = { x: (right.x + left.x) * 0.5, y: foot + height * 0.24 + lift };
  const rim = [
    apex,
    right,
    { x: heel.x + nook, y: foot + lift },
    heel,
    { x: heel.x - nook, y: foot + lift },
    left,
  ];
  ...
  /* Крен. На ходу вбок тело валится в сторону хода — как в игре. Поворачиваем
     сам контур, а не картинку: наклонённая грань иначе встречает воду, иначе
     её расталкивает и иначе срывает вихрь. Вершины apex, right и left лежат
...
function setupSwirl() {
  ...
  modeState.goUp = false;
  modeState.goDown = false;
  modeState.blobs = [];
  modeState.crowd = 0;
  modeState.shed = 0;
  modeState.panelMark = '';
...
function swirlPanels(body) {
  ...
  swirlBalance(panels);
  modeState.panels = panels;
  modeState.panelMark = mark;
  return panels;
}
...
function swirlShed(body, panels, blobs, flow, sway, beat) {
  /* Углы срывают по очереди, а не разом. Одновременный сход давал пару
     встречных вихрей, поля которых друг друга гасят: вода за кормой не
     закручивалась, а тормозила, и «завихрение» на вид не делало ничего при
     любом значении. Дорожка Кармана и начинается с того, что срыв с одной
     стороны подавляет срыв с другой, — поэтому чередуем, а каждый вихрь
     несёт всю завихренность, накопленную за такт. */
  modeState.side = -(modeState.side || 1);
...
function stepSwirl() {
  ...
  if (standing) {
    ...
    const beat = (0.8 * body.spread) / Math.max(flow, 0.02);
    if (modeState.shed >= beat) {
      modeState.shed = 0;
      swirlShed(body, panels, blobs, flow, sway, beat);
    }
  }
  ...
  for (const part of parts) {
    ...
    /* Отклонение считаем от настоящего сноса, а он на ходу вбок равен
       (-ход, -течение), а не просто «вниз». Со сносом «вниз» у всей реки
       разом появлялась поперечная составляющая величиной с ход, свечение
       вспыхивало по всему кадру и означало не завихрение, а то, что мы жмём
       на стрелку. */
    part.swing = Math.hypot(vx + sway, vy + flow) / Math.max(flow, 0.02);
    part.x += vx * STEP;
    part.y += vy * STEP;
...
function drawSwirl(hush) {
  ...
  if (modeState.age - (modeState.told || 0) > 0.5) {
    ...
    modeState.tally = modeState.blobs.length;
  }
...
function gameLevel() {
  /* Вал каждые четыре очка, а не шесть: давление должно начинаться раньше,
     пока игрок ещё не привык к спокойной воде. */
  return Math.floor(modeState.score / 4);
}
...
function stepGame() {
  ...
  /* Вал — каждые шесть очков. Раньше всё упиралось в потолки: после шестого
     вала состав вещей уже не менялся, после десятого — и частота, дальше
     росла одна скорость, и на сотне очков игра переставала расти вовсе.
     Теперь давит с четырёх сторон, и три из них не имеют потолка. */
  const level = gameLevel();
  modeState.pace = 1 + level * 0.13;
  modeState.churn = 1 + level * 0.2;
  modeState.grow = Math.min(1 + level * 0.06, 2.6);
  stepSwirl();
  ...
  if (!modeState.over) {
    modeState.drop += STEP;
    /* Частота падает долей, а не вычитанием: вычитание упиралось в пол на
       десятом вале, а доля жмёт и на сороковом. */
    const beat = Math.max(0.85 * Math.pow(0.93, level), 0.17);
    if (modeState.drop >= beat) {
      modeState.drop = 0;
      /* Крестов со временем больше, но не больше половины с небольшим: иначе
         ловить становится нечего и игра превращается в бег от всего. Дальше
         давит не доля, а кучность — кресты идут парами, потом тройками, и
         между ними надо продеться. */
      const harm = Math.min(0.18 + level * 0.04, 0.58);
      const roll = Math.random();
      const kind = roll < harm ? 'крест' : roll < harm + (1 - harm) * 0.62 ? 'клин' : 'круг';
      const bunch = kind === 'крест' && level >= 5 && Math.random() < 0.5 ? (level >= 11 ? 3 : 2) : 1;
      const at = 0.08 + Math.random() * 0.84;
      for (let i = 0; i < bunch; i += 1) {
        modeState.items.push({
          x: at + (i - (bunch - 1) * 0.5) * 0.055,
          y: 1.06 + i * 0.02,
          kind,
...
MODES.game = {
  label: 'игра',
  note: 'Та же река и та же буква, но по течению плывут вещи. Стрелки ведут Л: влево-вправо поперёк реки, вверх-вниз — прибавить и убавить ход. Клин — очко, крест — жизнь, кругляш не даёт ничего и стоит в наборе нарочно: без нейтральной вещи выбор свёлся бы к «хватай всё, что не крест». Жизней три. Ничего не падает по расписанию: вещи несёт то же поле, что и трассеры, поэтому у носа они обтекают букву, а в вихрях заворачивают — крест можно объехать, но можно и подвести к себе течением. Каждые четыре очка — новый вал, и давит он с четырёх сторон: река ускоряется, вещи идут чаще, крестов среди них больше и река становится вихрастее — вещи носит непредсказуемее. Доля крестов упирается в половину с небольшим, иначе ловить станет нечего; дальше давит кучность — кресты идут парами с пятого вала, а с одиннадцатого тройками, и между ними надо продеться. И сама Л прибавляет в росте: мишень крупнее, увернуться труднее — этим игра и кончается. Красным помечено происшествие, а не положение дел: удар о крест держится полсекунды и гаснет. «Заново» и клавиша R начинают сначала.',
  cursor: 'crosshair',
  tools: [
    { type: 'range', key: 'speed', label: 'течение', min: 0.15, max: 1.8, step: 0.05, value: 0.65 },
