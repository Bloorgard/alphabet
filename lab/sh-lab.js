...
/* Шаг стопки чуть меньше видимой высоты лежащего кольца: они садятся друг
   на друга внахлёст, как настоящие, а не висят распорками. */
const SH_SLOT_H = 0.04;
...
/* Люфт — это настоящий зазор между просветом кольца и стержнем. Больше него
   брать нельзя: кольцо начинает съезжать со штыря на глазах. */
const SH_LUFT = (SH_RING_R - SH_RING_LINE) - SH_THICK / 2;
const SH_LEAN = 1.1;       /* крен от собственного хода: кольцо ведёт за буквой */
...
function shHitTooth(ring) {
  ...
  for (let i = 0; i < list.length; i += 1) {
    ...
    /* Окно несимметрично: сверху оно кончается там же, где начинается срыв со
       стержня, снизу — шире. Иначе кольцо ловилось выше, чем может держаться,
       и приходилось дёргать его вниз — тот самый щелчок при посадке. */
    const atTip = ring.y > SH_TOP - SH_RING_R * 0.4 && ring.y < SH_TOP + SH_RING_R * 1.4;
    ...
    if (axial && flat && ring.vy > -0.03 && atTip && slip < SH_SWIPE) {
      /* Ход кольца при посадке сохраняется: оно продолжает опускаться с той же
         скоростью, а стержень лишь забирает у него свободу вбок. Обнуление
         всего разом и читалось как вырезанные кадры. */
      ring.pinned = { tooth: i };
      ring.vx *= 0.3;
      ring.spin *= 0.3;
      return true;
    ...
    /* Соосному кольцу зубец не мешает ни снизу вверх, ни на подлёте сверху:
       над кончиком там просто нет материала. Без этого оставалась узкая
       полоса прямо над зубцом, где падающее кольцо сносило с оси, и до
       посадки оно уже не доходило. */
    if (axial && (ring.vy <= 0 || ring.y < SH_TOP)) return false;
...
function shStack(ring) {
  ...
  ring.x += ring.vx * STEP;
  ring.vx -= ring.vx * SH_DRAG_X * 1.6 * STEP;
  const slack = ring.x - tooth;
  if (Math.abs(slack) > SH_LUFT) {
    const edge = Math.sign(slack) * SH_LUFT;
    ring.x = tooth + edge;
    ring.vx = (modeState.rigV - ring.vx) * 0.25;
  }

  /* Крен идёт от собственного хода: кольцо заваливается в ту сторону, куда
     его тащит стержень, и выравнивается, когда буква останавливается. */
  const lean = clamp((modeState.rigV - ring.vx) * SH_LEAN * 0.08, -0.22, 0.22);
