...
/* Скругление фронтов — это ограничение полосы: прибор усредняет сигнал по
   окну, и углы съедаются так же, как у живого осциллографа. Окно треугольное,
   иначе угол не скругляется, а срезается фаской. */
...
function sawLevel(phase, symmetry, band) {
  ...
  for (let k = 0; k < BAND_WINDOW.length; k += 1) {
    sum += BAND_WINDOW[k] * sawRaw(phase + band * (k / (BAND_WINDOW.length - 1) - 0.5), symmetry);
  }
  return sum / BAND_SUM;
}
